...
class ChessVisualizer:
    def __init__(self, model_path, video_name='chess_game.mp4', gif_name='chess_game.gif', video_frame_size=(1600, 1200), video_fps=0.0075):
        self.gif_frames = []
        ...
        self.model.eval()
        self.architecture = network.TrueNet(num_resBlocks=8, num_hidden=64, head_channel_policy=8, head_channel_values=4).to(self.device).to(torch.float32)
    ...
    def generate_gif(self):
        ...
        with torch.no_grad():
            while not board.is_game_over():
                bigl = []
                input_data = decoder.convert_board(board, bigl).unsqueeze(0).to(self.device)
                ...
                hook_handle_startblock = self.architecture.startBlock.register_forward_hook(self.hook_startblock)
                hook_handle_policy = self.architecture.policyHead.register_forward_hook(self.hook_policy)
                hook_handle_value = self.architecture.valueHead.register_forward_hook(self.hook_value)
                hook_handles_backbone = [self.architecture.backBone[i].register_forward_hook(self.hook_backbone) for i in range(8)]
                ...
                plt.imshow(self.activations_startblock[0][0].detach().cpu().numpy().reshape(64, 64), cmap='plasma')
                plt.axis('off')
                ...
                for i in range(len(self.activations_value)):
                    ...
                    value, _, _, _, _, _, best_move = decoder.decode_nn_output(self.activations_value[i], self.activations_policy[i], board)
                ...
                for i in range(len(self.activations_policy)):
                    plt.subplot(2, 1, 2)
                    _, _, _, _, legal_lookup, _, best_move = decoder.decode_nn_output(self.activations_value[i], self.activations_policy[i], board)
