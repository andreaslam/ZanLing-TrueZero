...
use tch::Tensor;
use tzrust::{data_path_str, decoder::eval_state, mcts_trainer::Net};

fn main() {
    env::set_var("RUST_BACKTRACE", "1");
    let net = Net::new(&data_path_str("nets/tz_6515.pt"));
    let mut avg = Vec::new();
    ...
    for _ in 0..10 {
        let data = Tensor::from_slice(&[0.0_f32; 1344 * BATCH_SIZE]); // 8*8*21 = 1344
    ...
    for _ in 0..100 {
        ...
        let eps = BATCH_SIZE as f32 / (elapsed_time_ms / 1e3);
