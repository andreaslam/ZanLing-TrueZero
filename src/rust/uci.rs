...
use crate::{
    ...
    cache::CacheEntryKey,
    debug_print,
    decoder::{convert_board, eval_state, extract_policy},
    ...
    loop {
        ...
        match *commands.first().unwrap_or(&"oops") {
            ...
            "eval" => {
                ...
                let (value, policy) = eval_state(convert_board(&bs), &net).unwrap();
                ...
                // policy head distribution, straight from the net (no search)
                ...
                let pol_logits: Vec<f32> = idx_li.iter().map(|&id| policy[id]).collect();
                let sm = Tensor::softmax(&Tensor::from_slice(&pol_logits), 0, Kind::Float);
                let pol_probs: Vec<f32> = Vec::try_from(sm).expect("Error");
                let mut move_probs: Vec<(Move, f32)> = legal_moves
                    .iter()
                    .zip(pol_probs.iter())
                    .map(|(mv, p)| {
                        let mut mv = *mv;
                        ...
                        (check_castling_move_on_output(&bs, mv), *p)
                    })
                    .collect();
                move_probs.sort_by(|a, b| b.1.partial_cmp(&a.1).unwrap());
                println!("policy:");
                for (mv, p) in move_probs.iter() {
                    println!("{:#} {:.2}%", mv, p * 100.0);
                }
            }
