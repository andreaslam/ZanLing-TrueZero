...
def play_game(NUMBER_OF_GAMES):
    for _ in range(NUMBER_OF_GAMES):
        ...
        if np.random.rand() > 0.5:
            ...
            model1_turn = -1
            model2_turn = 1
        print(model1_name, model1_turn, model2_name, model2_turn)
        ...
        while not board.is_game_over():
            legal_moves = list(board.legal_moves)
            m_dict = {}
            if colour == model1_turn:
                print(model1_name+"'s turn")
                for move in legal_moves:
                    test_board = board.copy()
                    test_board.push(move)
                    move_score = negamax_ab(
                        test_board, -np.inf, np.inf, -colour, model1, 2)
                    m_dict[str(move)] = move_score
            elif colour == model2_turn:
                print(model2_name+"'s turn")
                for move in legal_moves:
                    test_board = board.copy()
                    test_board.push(move)
                    move_score = negamax_ab(
                        test_board, -np.inf, np.inf, -colour, model2, 2)
                    m_dict[str(move)] = move_score
            if colour == -1:
                m_dict = {
                    k: v
