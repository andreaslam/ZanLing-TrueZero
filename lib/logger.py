...
class Logger:
    ...
    def log(self, group: str, name: str, value):
        ...
        assert np.isnan(
            a[self.curr_batch]
        ), f"Key {key} was already logged during this batch"
    ...
    def save(self, path: str):
        ...
        assert (
            path.suffix == ".npz"
        ), f"Log save path should have extension .npz, got {path}"
    ...
    def load(path) -> "Logger":
        ...
        result.data = {
            tuple(k): GrowableArray(v) for k, v in zip(data["keys"], data["values"])
        }
