...
class DataManager:
    ...
    def loading(self, train_data, train_target):
        ...
        X_train, X_val, y_train, y_val = train_test_split(
            train_data, train_target, test_size=0.8, shuffle=True
        )
...
class ELM(torch.nn.Module):
    def __init__(self, n_input, n_hidden, n_output):
        ...
        self.weight_output = torch.randn(n_hidden, n_output)
    ...
    def fit(self, x, y, num_iterations, learning_rate):
        ...
        # Compute hidden layer output
        self.hidden = torch.sigmoid(
            torch.matmul(x, self.weight_input) + self.bias_hidden
        )

        # Compute output layer weights using pseudoinverse
        pinv = torch.pinverse(self.hidden)
        self.weight_output = nn.Parameter(torch.matmul(pinv, y))

        # Perform additional iterations of training using secondary network
        for i in range(num_iterations):
            # Update weights using residues
    ...
    def update_weights(self, x, y, learning_rate):
        # Compute residues
        residues = self.compute_residues(x, y)
        residues = residues.view(-1, 1)
        # Update output layer weights
        self.weight_output.data += learning_rate * torch.matmul(
            self.hidden.T, residues
        ) 

        # Update output layer bias
        self.bias_output.data += learning_rate * residues.mean(dim=0)

        # Compute hidden layer gradients
        hidden_gradients = (
            torch.matmul(residues, self.weight_output.T)
            * self.hidden
            * (1 - self.hidden)
        )
        # Update input layer weights
        self.weight_input.data += learning_rate * torch.matmul(x.T, hidden_gradients)

        # Update input layer bias
        self.bias_hidden.data += learning_rate * hidden_gradients.mean(dim=0)
...
while all_completed == False:
    ...
    machine.fit(X_train, y_train, 5, 1e-4)
    ...
    torch.save(machine.state_dict(), 'elm2_model_weights'+str(counter)+'.pth')
    counter += 1
