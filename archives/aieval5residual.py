...
# aieval5 with residual characteristics
# imports
import numpy as np
import torch
...
model = nn.Sequential(
    ...
    nn.BatchNorm1d(256),
    nn.ReLU(),
    nn.Linear(256, 256),  # Residual connection
    nn.BatchNorm1d(256),
    nn.ReLU(),
    nn.Linear(256, 128),
    nn.Dropout(p=0.1),
    nn.BatchNorm1d(128),
    nn.LeakyReLU(0.01),
    nn.Linear(128, 128),  # Residual connection
    nn.BatchNorm1d(128),
    nn.ReLU(),
    nn.Linear(128, 64),
    nn.ReLU(),
...
scheduler = optim.lr_scheduler.ReduceLROnPlateau(
    optimizer, factor=0.01, patience=40, verbose=True
) # 0.01, 30
