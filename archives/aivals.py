...
class Agent(nn.Module):
    def __init__(self):
        ...
        # randomising the optimiser 
        decider = np.random.rand()
        if decider <= 0.25:
            self.optimizer = optim.Adam(self.parameters(), lr=1e-3)
        elif decider > 0.25 and decider <= 0.5:
            self.optimizer = optim.SGD(self.parameters(), lr=1e-3)
        elif decider > 0.5 and decider <= 0.75:
            self.optimizer = optim.Adamax(self.parameters(), lr=1e-3)
        else:
            self.optimizer = optim.Adadelta(self.parameters(), lr=1e-3)
...
def play_game(agent1, index):
    ...
    while counter < len(puzzle_soln):
        ...
        batch_targets.append(
            agent1.forward(torch.tensor(board_data(pre_board), dtype=torch.float)).item())
...
PUZZLE_SIZE = 3517080  # insert actual number of puzzles (starting from 1)
