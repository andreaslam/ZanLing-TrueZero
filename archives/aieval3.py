...
try:
    train_data = torch.load("./train_data.pckl")
    train_target = torch.load("./train_target.pckl")
...
)  # reshape y_train to match the input size
X_val = torch.tensor(X_val, dtype=torch.float32)
y_val = torch.tensor(
    y_val, dtype=torch.float32
)  # reshape y_val to match the input size
...
n_epochs = 50  # number of epochs to run optimal = 40, 220
