...
class Agent(nn.Module):
    ...
    def generate_move(self, board, color):
        ...
        scores = []
        raw_data = negamax_alpha_beta(self, board, BEAM_DEPTH, -np.inf, np.inf, color)
        print("RAW", raw_data)
        scores.append(raw_data)
        print("SCORES", scores)
        best_moves_idx = np.argsort(scores)[-BEAM_WIDTH:]
        print(best_moves_idx)
        best_move_idx = np.random.randint(0, len(best_moves_idx))
        print(best_move_idx)
        with open("./gamescopy.txt", "a+") as f:
...
def negamax_alpha_beta(agent, board, depth, alpha, beta, color):
    if depth == 0:
        ...
        return (
            color
            * agent.forward(torch.tensor(board_data(board), dtype=torch.float)).item()
        )
    ...
    if board.is_game_over():
        if board.is_stalemate():
            ...
            return -color * 10000  # large negative value for checkmate
    ...
    for move in legal_moves:
        board.push(move)
        score = -negamax_alpha_beta(agent, board, depth - 1, -beta, -alpha, -color)
        board.pop()
    ...
    for i in range(len(population)):
        for j in range(i + 1, len(population)):
            ...
            while not board.is_game_over():
                ...
                if decider > 0.5:
                    move = agent1.generate_move(board, color)
                else:
                    move = agent2.generate_move(board, color)
                board.push(move)
                if decider > 0.5:
    ...
    if is_winning is True:
        ...
        for agent_index in range(num_of_saves):
            torch.save(
                ...
                "./best_agentscopy" + str(agent_index) + ".pt",
            )
