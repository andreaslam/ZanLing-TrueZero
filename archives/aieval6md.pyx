...
class DataManager:
    ...
    def load(self, completed, size):
        ...
        for g in tqdm.tqdm(games, desc="each game"):
            game = g[2]
            MAX_MOMENTS = min(20, len(game)) # is 20 positions more or is the length of the game more? 
            # if there are less than 20 moves in a game, why index 20 positions?
            unsampled_idx = [
...
class Train(Tanh200):
    ...
    def cycle(self, X_train, y_train, X_val, y_val, l_r):
        ...
        scheduler2 = torch.optim.lr_scheduler.CosineAnnealingLR(
            optimizer, T_max=1000, verbose=True
        )
        ...
        #     scheduler = optim.lr_scheduler.ReduceLROnPlateau(
        #         optimizer, factor=0.0000000000000001, patience=5, verbose=True
        #     )
        #     scheduler2 = torch.optim.lr_scheduler.CosineAnnealingLR(
        #         optimizer, T_max=50, verbose=True
        #     )
        #     n_epochs = 200  # number of epochs to run optimal = 40, 220
        batch_start = torch.arange(0, len(X_train), batch_size)
        # Hold the best model
        best_mse = np.inf  # initialise value as infinite
        best_weights = None
        history = []
        accumulation_steps = 5  # accumulate gradients over 4 batches
...
while all_completed == False:
    ...
    t.cycle(X_train, y_train, X_val, y_val, 7.5e-4)
